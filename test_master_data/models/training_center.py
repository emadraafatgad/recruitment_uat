...
class TrainingList(models.Model):
    ...
    training_center = fields.Many2one('training.center',track_visibility="onchange",required=True)
    ...
    def create_bill(self):
        ...
        cr = self.env['account.invoice'].create({
            'partner_id': self.training_center.partner_id.id,
            'currency_id': product.currency_id.id,
            'state': 'draft',
            'type': 'in_invoice',
            'partner_type':self.training_center.partner_id.vendor_type,
            'origin': self.name,
            'journal_id': purchase_journal.id,
