...
class account_statement_report(models.TransientModel):
	...
	def print_report(self):
		...
		return self.env['report'].get_action(self, 'gt_customer_account_statement.cus_acc_report')
	...
	def lines(self, partner_id):
		...
		if movelines:
			for line in movelines:
				...
				date = line.date
				condition_1 = (datetime.strptime(str(date), '%Y-%m-%d') >= datetime.strptime(str(self.period_from), '%Y-%m-%d'))
				condition_2 = (datetime.strptime(str(date), '%Y-%m-%d') <= datetime.strptime(str(self.period_to), '%Y-%m-%d'))
				diff_date=datetime.strptime(str(self.period_to), '%Y-%m-%d') - datetime.strptime(str(date), '%Y-%m-%d')
				diff=diff_date.days
			...
			for invoice_id in inv_ids:
				...
				for line_b in s_final:
					...
					if condition_1 and  condition_2:
						...
						diff_date=datetime.strptime(str(self.period_from), '%Y-%m-%d') - datetime.strptime(str(date), '%Y-%m-%d')
						diff=diff_date.days
						pass_amount = line_b.debit - line_b.credit
						for pay_b in payments:
							if pass_amount > 0.0:
								date = pay_b.date
								condition_3 = (datetime.strptime(str(date), '%Y-%m-%d') >= datetime.strptime(str(self.period_from), '%Y-%m-%d'))
								condition_4 = (datetime.strptime(str(date), '%Y-%m-%d') <= datetime.strptime(str(self.period_to), '%Y-%m-%d'))
