...
class Partner(models.Model):
    ...
    vendor_type = fields.Selection([('supplier', 'supplier'), ('agent', 'Agent'), ('nira_broker', 'Nira Broker'),
                                    ('passport_broker', 'Passport Broker')
                                       , ('passport_placing_issue', 'Passport Placing Issue'),
                                    ('interpol_broker', 'Interpol Broker'),
                                    ('gcc', 'Gcc'), ('hospital', 'Hospital'), ('enjaz', 'Enjaz'),
                                    ('embassy', 'Embassy'), ('lab', 'Lab'), ('travel_company', 'Travel Company'),
    ...
    def constrains_cost(self):
        if self.vendor_type in (
        'passport_broker', 'interpol_broker', 'passport_placing_issue', 'hospital') and self.cost <= 0:
